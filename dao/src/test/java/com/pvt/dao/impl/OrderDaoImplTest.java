...
//        }
//        util.getSession().getTransaction().commit();
//        List<Order> orderListActual = OrderDaoImpl.getInstance().getClientOrdersListByStatus(OrderStatus.REQUESTED, user1);
//        Assert.assertTrue(orderListExpected.containsAll(orderListActual) && orderListActual.containsAll(orderListExpected));
//    }
...
//        util.getSession().getTransaction().commit();
//        Order actual = OrderDaoImpl.getInstance().get(expected.getOrderId());
//        Assert.assertEquals(expected, actual);
//    }
